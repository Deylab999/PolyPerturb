...
for(traitname in common_traits){
  ...
  set.seed(1)
  tt = ppi_string_RWR(cbind(as.matrix(qdff)[,1], as.matrix(qdff)[,1]), NUM_RUNS = 5)
...
write.table(ccmat, file = paste0("/n/groups/price/kushal/GeneRank/Genes_by_X/Network_plus/PPI_STRING_MAGMA_SAIGE_pooled.txt"),
            row.names = T, col.names = T, sep = "\t", quote=F)


#######################################  PPI-STRING-MAGMA-SAIGE-indiv   #####################################################################
...
rownames(ccmat) = common_genes
colnames(ccmat) = common_traits
write.table(ccmat, file = paste0("/n/groups/price/kushal/GeneRank/Genes_by_X/Network_plus/PPI_STRING_MAGMA_SAIGE_indiv.txt"),
            row.names = T, col.names = T, sep = "\t", quote=F)
