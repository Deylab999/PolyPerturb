...
# matt = as.data.frame(dorothea_hs)
# matt = matt[which(matt$mor == 1), ]
# matt2 = cbind.data.frame(matt$tf, matt$target, matt$mor)
# write.table(matt2, file = "/n/groups/price/kushal/extras/edgelists/dorothea_hs_all_poslinks",
#              row.names = F, col.names = F, sep = "\t", quote=F)

#######################################  PPI-Dorothea-MAGMA-PoPS   #####################################################################
...
gg_edgelist = read.table("/n/groups/price/kushal/extras/edgelists/dorothea_hs_all_poslinks", header=F)
...
write.table(ccmat, file = paste0("/n/groups/price/kushal/GeneRank/Genes_by_X/Network_plus/PPI_Dorothea_MAGMA_PoPS.txt"),
...
rownames(ccmat) = common_genes
colnames(ccmat) = common_traits
write.table(ccmat, file = paste0("/n/groups/price/kushal/GeneRank/Genes_by_X/Network_plus/PPI_Dorothea_MAGMA.txt"),
            row.names = T, col.names = T, sep = "\t", quote=F)
...
rownames(ccmat) = common_genes
colnames(ccmat) = common_traits
write.table(ccmat, file = paste0("/n/groups/price/kushal/GeneRank/Genes_by_X/Network_plus/PPI_Dorothea_PoPS.txt"),
            row.names = T, col.names = T, sep = "\t", quote=F)
